...
def laplace_1d_eigen(x, N, L=1.):
    ...
    u_k(x) = sqrt(L/2) * sin( k * pi / L * x),
    lambda_k = -(k * pi / L)**2

    where the factor sqrt(L/2) is chosen to normalize the eigenfunctions.

    :param x: Positions at which u_k are evaluated.
    :param N: Number of eigenvectors and eigenvalues
    :param L: Length of the domain
    :return: (u_k, lambda_k). u_k is an array with shape (N, x.size), lambda_k is a vector of length N
    """
    k = np.linspace(1, N, N)
    arg = k * np.pi / L
