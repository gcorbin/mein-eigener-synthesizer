...
def main():
    ...
    opts = InstrumentOptions(base_frequency=args.base_frequency, L=1., Nk=args.basis_functions, Nx=100, pick_pos=args.pick_pos)
    instrument = instruments[args.instrument](opts)
...
def show_sound(instrument, args):
    samplerate = 44100  # Hz
    duration = 20 * instrument.options.halflife
    t = samples(samplerate, duration)
    #x_out = instrument.options.L * np.arange(0.6, 0.8, 0.02)
    x_out = args.pickup_pos * instrument.options.L
    sound = np.sum(instrument.sound(t, x_out), axis=1)
...
def animate_initial_condition(instrument, args):
    U0 = instrument.sound(np.array([0.]), instrument.x)

    fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True)
    # Plot the individual basis functions
    lines1 = []
    for i in range(instrument.options.Nk):
        line, = ax1.plot(instrument.x, np.zeros_like(instrument.x), label=f'e_{i}')
        lines1.append(line)
    ax1.set_xlabel('x / m')
    ax1.set_ylabel('u')
    plot_scale = 1.1 * np.max(np.abs(instrument.e_k))
    ax1.set_ylim((-plot_scale, plot_scale))

    # Plot the sum of basis functions
    line2, = ax2.plot(instrument.x, np.zeros_like(instrument.x), label=f'U0 with {0} basis functions')
    ax2.plot(instrument.x, U0[0, :], label=f'U0 with {instrument.options.Nk} basis functions')
    ax2.legend()
    ax2.axvline(x=instrument.options.pick_pos, color="black", linestyle="--")
    ax2.set_xlabel('x / m')
    ax2.set_ylabel('u')
    ...
    def init_animation():
        for k in range(len(lines1)):
            lines1[k].set_ydata(np.zeros_like(instrument.x))
        line2.set_ydata(np.zeros_like(instrument.x))
    ...
    ani = animation.FuncAnimation(fig,
                                  ...
                                  interval=1000,
                                  frames=instrument.options.Nk+1,
                                  blit=True,
                                  repeat_delay=2000)
...
def animate_vibration(instrument, args):
    samplerate = 44100  # Hz
    duration = 5. / args.base_frequency
    t = samples(samplerate, duration)
    U = instrument.sound(t, instrument.x)

    fig, ax = plt.subplots()
    ax.set_xlabel('x / m')
    ax.set_ylabel('u')
    ax.axvline(x=instrument.options.pick_pos, color="black", linestyle="--")
    plot_scale = np.max(np.abs(U[0, :]))
    ax.set_ylim((-1. * plot_scale, plot_scale))
    line, = ax.plot(instrument.x, U[0, :])
